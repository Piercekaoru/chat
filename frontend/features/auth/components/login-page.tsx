...
type LoginPanelProps = {
  nextPath: string;
  /**
   * When provided, successful login/registration invokes this callback instead of
   * navigating to `nextPath`. Used by the in-place login dialog (guest mode).
   */
  onAuthenticated?: () => void;
  /** Hide the brand mark (e.g. when the dialog header already shows branding). */
...
/**
 * The login/registration/reset form without any full-screen chrome. Reused by the
 * standalone login page and the in-place login dialog shown to guests.
 */
export function LoginPanel({ nextPath, onAuthenticated, showBrandMark = true, className }: LoginPanelProps) {
  const t = useTranslations("login");
  const loginPage = useLoginPage({ nextPath, onAuthenticated });
  const {
  ...
  return (
    <div className={cn("w-full max-w-[360px]", className)} aria-busy={!configReady}>
      {showBrandMark ? <LoginBrandMark /> : null}
