...
const CHAT_COMPOSER_SELECTION_STORAGE_KEY = "openachieve:chat-composer-selection:v1";
...
function readSelectionStore(): PersistedComposerSelectionStore {
  ...
  try {
    const raw = readLocalStorageItem(CHAT_COMPOSER_SELECTION_STORAGE_KEY, LEGACY_CHAT_COMPOSER_SELECTION_STORAGE_KEY);
    if (!raw) {
      return {};
    }
...
function writeSelectionStore(store: PersistedComposerSelectionStore) {
  ...
  try {
    if (Object.keys(store).length === 0) {
      removeLocalStorageItem(CHAT_COMPOSER_SELECTION_STORAGE_KEY, LEGACY_CHAT_COMPOSER_SELECTION_STORAGE_KEY);
      return;
    }
    writeLocalStorageItem(CHAT_COMPOSER_SELECTION_STORAGE_KEY, JSON.stringify(store), LEGACY_CHAT_COMPOSER_SELECTION_STORAGE_KEY);
  } catch {
